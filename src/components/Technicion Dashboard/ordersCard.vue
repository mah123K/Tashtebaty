...
<template>
  <div class="order rounded-2xl shadow-md p-5 md:w-[31%] bg-white dark:bg-[#16222B] m-2 relative ">
    ...
    <div class="element flex m-1 text-lg">
      ...
      class="w-[25px] h-[25px]"
      >
        <path fill="#2574b9" d="M0 188.6C0 84.4 86 0 192 0S384 84.4 384 188.6c0 119.3-120.2 262.3-170.4 316.8-11.8 12.8-31.5 12.8-43.3 0-50.2-54.5-170.4-197.5-170.4-316.8zM192 256a64 64 0 1 0 0-128 64 64 0 1 0 0 128z"/>              
      </svg>
      <p class="mx-1 text-[#133B5D] dark:text-white">
        <span class="font-bold text-[#133B5D] dark:text-white">
    ...
    <div class="buttonsContainer flex w-[50%] mx-auto justify-between mt-5">
      <div @click="handleAccept" class="cursor-pointer acceptIcon rounded-full w-[60px] h-[60px] bg-green-500 flex flex-col items-center justify-center text-white">
        ...
        <p class="text-[10px] font-bold">{{ texts[lang].technicianDashboard.buttons?.accept || 'Accept' }}</p>
      </div>
      ...
      <div @click="handleDecline" class="cursor-pointer declineIcon rounded-full w-[60px] h-[60px] bg-red-500 flex flex-col items-center justify-center text-white">
        ...
        <p class="text-[10px] font-bold">{{ texts[lang].technicianDashboard.buttons?.decline || 'Decline' }}</p>
      </div>
    </div>
  </div>
    ...
    <div v-if="showReasonPopup" class="fixed inset-0 bg-[#0000008a] flex justify-center items-center z-50">
      <div class="bg-white dark:bg-[#16222B] dark:text-white p-6 rounded-2xl shadow-xl w-[400px] text-center">
        <h3 class="text-xl font-semibold mb-4 text-[#133B5D] dark:text-white">
          {{ texts[lang].technicianDashboard.messages?.enterDeclineReason || 'Enter reason for cancellation' }}
        </h3>
        <textarea v-model="declineReason" rows="4" class="w-full border rounded-lg p-2 dark:bg-[#16222B] dark:text-white border-gray-300 focus:ring-2 focus:ring-[#133B5D]" :placeholder="texts[lang].technicianDashboard.messages?.typeReasonPlaceholder || 'Type the reason...'"></textarea>
        <div class="flex justify-end gap-3 mt-4">
          <button @click="cancelDecline" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded-lg">
            {{ texts[lang].technicianDashboard.buttons?.cancel || 'Cancel' }}
          </button>
          <button @click="confirmDecline" class="bg-[#133B5D] hover:bg-[#1b5383] text-white px-5 py-2 rounded-lg font-semibold">
  ...
  >
    ...
    >
      ...
      <div class="mt-4 space-y-2 text-lg">
        <textarea disabled class="text-[#133B5D] dark:text-white border-[#133B5D] border-2 p-2 rounded-xl w-full h-[130px] disabled">
          {{ order.descreption }}
        </textarea>
        ...
        <div v-if="order.imageUrls && order.imageUrls.length" class="mt-4">
          <p class="font-bold text-[#133B5D] dark:text-white mb-2">
            {{ texts[lang].technicianDashboard.ordersCard?.attachedPhotos || 'Attached Photos:' }}
          </p>
